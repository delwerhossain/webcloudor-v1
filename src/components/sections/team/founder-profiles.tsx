...
const FounderProfile = ({ data, imagePosition, delay = 0 }: FounderProfileProps) => {
  ...
  return (
    <div ref={ref} className={`grid lg:grid-cols-2 gap-12 lg:gap-16 items-start ${imagePosition === "right" ? "lg:grid-flow-col-dense" : ""}`}>
      {/* Professional Photo */}
      <motion.div
      ...
      <motion.div
        ...
        className={`space-y-8 ${imagePosition === "right" ? "lg:col-start-1 lg:row-start-1" : ""}`}
      >
        {/* Header */}
        <div className="space-y-4">
          <Badge variant="secondary" className="text-[#00A8E8]">{data.role}</Badge>
          <h3 className="text-3xl md:text-4xl font-bold text-[#0A0A0B]">{data.name}</h3>
          <p className="text-lg text-[#64748B] leading-relaxed">{data.bio}</p>
        </div>

        {/* Quote */}
        <Card className="p-6 bg-[#F8FAFC] border-l-4 border-[#00A8E8]">
          <blockquote className="text-[#64748B] italic leading-relaxed">
            "{data.quote}"
          </blockquote>
        </Card>
        ...
        {/* Core Expertise */}
        <div className="space-y-4">
          <h4 className="text-xl font-semibold text-[#0A0A0B]">Core Expertise</h4>
          <div className="space-y-3">
            {data.expertise.map((item, index) => (
              ...
              >
                <span className="font-medium text-[#0A0A0B] min-w-fit">{item.category}:</span>
                <span className="text-[#64748B]">{item.skills}</span>
              </motion.div>
            ))}
          </div>
        </div>
        ...
        <div className="space-y-4">
          <h4 className="text-xl font-semibold text-[#0A0A0B]">Professional Background</h4>
          <div className="grid sm:grid-cols-2 gap-3">
            {data.background.map((item, index) => (
              ...
              >
                <span className="font-medium text-[#00A8E8] text-sm">{item.title}</span>
                <span className="text-[#64748B] text-sm">{item.detail}</span>
              </motion.div>
            ))}
          </div>
        </div>
        ...
        {/* Key Achievements */}
        <div className="space-y-4">
          <h4 className="text-xl font-semibold text-[#0A0A0B]">Key Achievements</h4>
          <ul className="space-y-2">
            {data.achievements.map((achievement, index) => (
              ...
              >
                <span className="text-[#00A8E8] mt-1.5 text-xs">•</span>
        ...
        >
          ...
          <Button 
            variant="outline" 
            size="sm"
            className="text-[#64748B] border-[#E2E8F0] hover:border-[#00A8E8]"
          >
            LinkedIn
          </Button>
          <Button 
            ...
            className="text-[#64748B] border-[#E2E8F0] hover:border-[#00A8E8]"
          >
            {data.contact.github ? "GitHub" : "Email"}
          </Button>
        </motion.div>
      </motion.div>
    </div>
